...
module Repositories
  ...
  class TopicTypes < ROM::Repository[:topic_types]
    ...
    def all(opts=false)
      opts ? aggregate([:topic_type_opts]).to_a : root.to_a
    end
    ...
    def with_opts(id)
      aggregate([:topic_type_opts]).by_id(id).one
    end
    ...
    def by_name(value, opts=false)
      opts ? aggregate([:topic_type_opts]).by_name(value).one : root.by_name(value).one
    end

    def find_by(conditions, opts=false)
      opts ? aggregate([:topic_type_opts]).find_by(conditions).to_a : root.find_by(conditions).to_a
    end
